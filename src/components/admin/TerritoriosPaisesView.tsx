...
export const TerritoriosPaisesView: React.FC<TerritoriosPaisesViewProps> = ({
  ...
  return (
    <div className="w-full max-w-[1600px] mx-auto px-3.5 sm:px-5 lg:px-6 py-4 sm:py-6 space-y-5 lg:space-y-6">
      ...
      {/* ========================================================================= */}
      {/* 2. TOP KPI ROW: 6 CARDS (Padrão Proporcional e Refinado)                  */}
      {/* ========================================================================= */}
          ...
          >
            ...
            <div className="flex items-center justify-between gap-1.5">
              <div className={`w-8 h-8 rounded-xl ${kpi.bgClass} flex items-center justify-center shrink-0 group-hover:scale-105 transition-transform shadow-2xs`}>
                {kpi.icon === 'globe' && <Globe className={`w-4 h-4 ${kpi.iconClass}`} strokeWidth={2.2} />}
                {kpi.icon === 'mappin' && <MapPin className={`w-4 h-4 ${kpi.iconClass}`} strokeWidth={2.2} />}
                {kpi.icon === 'building' && <Building2 className={`w-4 h-4 ${kpi.iconClass}`} strokeWidth={2.2} />}
                {kpi.icon === 'flag' && <Flag className={`w-4 h-4 ${kpi.iconClass}`} strokeWidth={2.2} />}
                {kpi.icon === 'plus' && <PlusCircle className={`w-4 h-4 ${kpi.iconClass}`} strokeWidth={2.2} />}
                {kpi.icon === 'pie' && <PieChart className={`w-4 h-4 ${kpi.iconClass}`} strokeWidth={2.2} />}
              </div>
              <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded-md whitespace-nowrap shadow-2xs ${
                kpi.id === 'kpi-cobertura'
                  ? 'text-purple-700 bg-purple-50/90 border border-purple-200/60'
                  : 'text-emerald-700 bg-emerald-50/90 border border-emerald-200/60'
              }`}>
                {kpi.trend}
              </span>
            </div>

            {/* Conteúdo Central: Métrica Proporcional + Rótulo sem Cortes */}
            <div className="mt-3">
