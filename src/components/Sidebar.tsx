...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  // 10 exact navigation items and custom icons matching reference design
  const navItems = [
    {
      id: 'inicio',
      label: 'Início',
  ...
  return (
    <>
      ...
      <aside
        id="main-sidebar"
        className={`fixed top-0 left-0 bottom-0 z-50 w-64 h-screen bg-surface-sidebar border-r border-accent-border px-3.5 py-3.5 flex flex-col justify-between overflow-hidden select-none transition-transform duration-300 ease-in-out lg:translate-x-0 ${
          isMobileOpen ? 'translate-x-0' : '-translate-x-full'
        } shadow-xs`}
      >
        {/* Top Section: Logo + Slogan & Navigation Menu */}
        <div className="flex flex-col gap-2 min-h-0">
          ...
          {/* 2. Navegação Principal (10 itens compactos) */}
                  ...
                  <div className="flex items-center gap-2.5 min-w-0">
                    <span
                      className={`shrink-0 transition-colors ${
                        isActive ? 'text-white' : 'text-ink group-hover:text-brand-blue'
                      }`}
                    >
        ...
        {/* Bottom Section: Promo Globe Card + Settings + Auth Buttons */}
        <div className="flex flex-col gap-2 pt-2 border-t border-accent-border/60 shrink-0">
          {/* 3. Card do Globo Terrestre */}
          <div
            id="sidebar-promo-card"
            className="p-2.5 rounded-xl bg-[#F8FAFC] border border-accent-border/80 relative overflow-hidden flex flex-col items-center text-center"
          >
            {/* Illustrated Stylized 3D Dotted Blue Earth Globe */}
            <div className="relative w-14 h-14 mb-1 flex items-center justify-center">
              <svg viewBox="0 0 100 100" className="w-full h-full">
                ...
                {/* Ambient glow circle */}
                <circle cx="50" cy="50" r="46" fill="url(#globe-glow)" />

                {/* Dotted globe outline */}
                <circle cx="50" cy="50" r="38" fill="#FFFFFF" stroke="#E2E8F0" strokeWidth="1" />

                {/* Continents */}
                <path
                  d="M32 30c3-4 8-6 14-4 4 1 6 5 5 9-1 4-5 6-9 7-4 1-9-1-10-5v-7z"
                  fill="#3B82F6"
                  opacity="0.85"
                />
                <path
                  d="M48 42c4-2 9-1 12 2 3 3 3 8 0 11-3 3-8 4-12 2-3-2-4-7-2-11l2-4z"
                  fill="#2563EB"
                />
                <path
                  d="M30 52c2-3 6-4 9-2 3 2 4 6 2 9-2 3-6 4-9 2-3-2-4-6-2-9z"
                  fill="#60A5FA"
                />
                <path
                  d="M62 28c3-2 7-1 9 2s1 7-2 9-7 1-9-2 0-7 2-9z"
                  fill="#3B82F6"
                  opacity="0.8"
                />
                <path
                  d="M65 55c2-2 5-1 7 1s1 5-1 7-5 1-7-1 0-5 1-7z"
                  fill="#2563EB"
                  opacity="0.9"
                />

                {/* Orbiting particles */}
                <circle cx="22" cy="38" r="1.5" fill="#3B82F6" opacity="0.6" />
                <circle cx="78" cy="44" r="1.5" fill="#2563EB" opacity="0.6" />
                <circle cx="42" cy="18" r="1.5" fill="#60A5FA" opacity="0.6" />
                <circle cx="58" cy="82" r="1.5" fill="#3B82F6" opacity="0.6" />
              </svg>
            </div>
            ...
            <button
              ...
              className="mt-1.5 inline-flex items-center gap-1 text-[10.5px] font-bold text-brand-blue hover:text-blue-700 transition-colors cursor-pointer group/impact"
            >
          ...
          <div className="flex items-center justify-between px-0.5">
            ...
            <div className="relative">
              <button
                ...
                className="flex items-center gap-1 px-1.5 py-0.5 text-[11px] font-bold text-ink hover:bg-slate-100 rounded-md transition-colors cursor-pointer"
                id="sidebar-lang-btn"
              >
              ...
              {isLangOpen && (
                <div className="absolute bottom-full mb-1 left-0 w-24 bg-white border border-accent-border rounded-xl shadow-lg py-1 z-30">
                  <button
          ...
          <div className="grid grid-cols-2 gap-2 pt-0.5">
            <button
              ...
              className="w-full py-1.5 px-2 rounded-xl border border-accent-border hover:bg-slate-50 font-bold text-xs text-ink transition-colors text-center cursor-pointer"
            >
              Entrar
            </button>
            <button
              ...
              className="w-full py-1.5 px-2 rounded-xl bg-[#0284C7] hover:bg-[#0369A1] text-white font-bold text-xs transition-colors shadow-2xs text-center cursor-pointer"
            >
              Criar Conta
            </button>
          </div>
