...
export const EnvironmentImpactView: React.FC<EnvironmentImpactViewProps> = ({
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] pb-16 antialiased text-[#1E293B]">
      ...
      <header className="sticky top-0 z-40 bg-white/95 backdrop-blur-md border-b border-slate-200/80 px-4 sm:px-6 lg:px-8 py-3">
        <div className="max-w-[1600px] mx-auto flex flex-col md:flex-row items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2.5 text-xs text-slate-500 w-full md:w-auto overflow-x-auto whitespace-nowrap">
            ...
            <button
              ...
              className="hover:text-[#064E3B] font-medium transition-colors cursor-pointer"
            >
              Comunidade Global
            </button>
          ...
          <div className="relative w-full md:w-96 lg:w-[480px]">
            ...
            <input
              ...
              placeholder="Pesquisar iniciativas, temas, organizações..."
              className="w-full bg-[#F1F5F9]/80 focus:bg-white text-xs pl-9 pr-14 py-2 rounded-full border border-slate-200 focus:border-[#064E3B] focus:ring-2 focus:ring-[#064E3B]/15 outline-none transition-all"
            />
          ...
          {/* Idioma, Notificações, Perfil */}
          <div className="flex items-center gap-3 self-end md:self-auto">
            {/* Seletor de Idioma */}
            <div className="relative">
              <button
                onClick={() => setIsLangMenuOpen(!isLangMenuOpen)}
                className="flex items-center gap-1.5 px-2.5 py-1.5 text-xs font-semibold text-slate-700 bg-white hover:bg-slate-50 border border-slate-200 rounded-lg shadow-2xs cursor-pointer"
              >
              ...
              {isLangMenuOpen && (
                <div className="absolute right-0 mt-1.5 w-32 bg-white border border-slate-200 rounded-xl shadow-lg p-1 z-50">
                  {(['PT', 'EN', 'ES'] as const).map((l) => (
                    <button
                      ...
                      className={`w-full text-left px-2.5 py-1 text-xs rounded-lg flex items-center justify-between cursor-pointer ${
                        selectedLang === l ? 'bg-emerald-50 text-[#064E3B] font-bold' : 'text-slate-600 hover:bg-slate-50'
                      }`}
                    >
                      <span>{l === 'PT' ? 'Português' : l === 'EN' ? 'English' : 'Español'}</span>
                      {selectedLang === l && <Check className="w-3 h-3 text-[#064E3B]" />}
                    </button>
                  ))}
                </div>
              )}
              ...
              {isNotificationsOpen && (
                <div className="absolute right-0 mt-2 w-80 bg-white border border-slate-200 rounded-2xl shadow-xl p-3 z-50 animate-in fade-in">
                  <div className="flex items-center justify-between pb-2 border-b border-slate-100">
                    <span className="text-xs font-bold text-slate-800">Notificações Ambientais</span>
                    <span className="text-[10px] bg-emerald-100 text-[#064E3B] px-1.5 py-0.5 rounded-full font-bold">3 novas</span>
                  </div>
                  <div className="mt-2 space-y-2 max-h-60 overflow-y-auto text-xs text-slate-600">
                    <div className="p-2 rounded-xl hover:bg-slate-50 transition-colors">
                      <p className="font-semibold text-slate-800">Novo projeto no Brasil aprovado</p>
                      <p className="text-[11px] text-slate-500">Reflorestamento de 2.000 hectares iniciado no Cerrado.</p>
                      <span className="text-[9px] text-slate-400">Há 25 minutos</span>
                    </div>
                    <div className="p-2 rounded-xl hover:bg-slate-50 transition-colors">
                      <p className="font-semibold text-slate-800">Meta de CO₂ alcançada</p>
                      <p className="text-[11px] text-slate-500">A comunidade atingiu 2.4M toneladas de carbono evitado.</p>
